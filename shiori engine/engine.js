...
const gameEngine = {
     ...
     * 3. 根据环境自动选择格式（HTML=MP4, Python=WebM）
    ...
    resolveVideoPath: function(videoKey) {
        ...
        // 检测运行环境，根据环境选择格式
        const isQtWebEngine = (typeof qt !== 'undefined' && qt.webChannelTransport);
        
        // Python环境优先使用.webm（避免MP4闪退），HTML环境优先使用.mp4
        const preferredExt = isQtWebEngine ? '.webm' : '.mp4';
        
        // 返回首选格式路径
        const preferredPath = basePath + videoKey + preferredExt;
        console.log('[Video] resolveVideoPath:', videoKey, '→', preferredPath, '(环境:', isQtWebEngine ? 'Python' : 'HTML', ')');
     ...
     * @param {boolean} isQtWebEngine - 是否在QtWebEngine环境中
     * @returns {boolean} - true表示可以播放，false表示应该跳过
     */
    checkVideoEnvironment: function(videoKey, isQtWebEngine) {
        // 如果没有video.html页面，无法获取环境信息，默认允许播放
        if (!this.videoEnvMap) {
            return true;
        }
        
        // 查找视频的环境配置
        const env = this.videoEnvMap[videoKey];
        
        if (!env) {
            // 未在video.html中配置的视频，默认允许播放（兼容性）
            return true;
        }
        
        // 检查环境匹配
        const currentEnv = isQtWebEngine ? 'python' : 'html';
        const isMatch = (env === currentEnv);
     ...
     * 从video.html加载视频环境映射
     */
    loadVideoEnvironmentMap: function() {
        const self = this;
        
        // 只在QtWebEngine环境中尝试加载
        if (!(typeof qt !== 'undefined' && qt.webChannelTransport)) {
            return;
        }
        
        // 通过QWebChannel从Python端获取视频环境映射
        if (typeof QWebChannel !== 'undefined') {
            new QWebChannel(qt.webChannelTransport, function(channel) {
                try {
                    if (channel.objects && channel.objects.VideoDecoder) {
                        const decoder = channel.objects.VideoDecoder;
                        
                        // 调用Python端的getVideoEnvMap方法
                        if (typeof decoder.getVideoEnvMap === 'function') {
                            try {
                                const result = decoder.getVideoEnvMap();
                                console.log('[VideoEnv] getVideoEnvMap返回类型:', typeof result);
                                console.log('[VideoEnv] getVideoEnvMap返回值:', result);
                                
                                // 检查是否为Promise
                                if (result && typeof result.then === 'function') {
                                    console.log('[VideoEnv] getVideoEnvMap返回Promise，等待解析...');
                                    result.then(function(finalResult) {
                                        console.log('[VideoEnv] Promise解析结果:', finalResult);
                                        if (finalResult) {
                                            try {
                                                self.videoEnvMap = JSON.parse(finalResult);
                                                console.log('[VideoEnv] 环境映射加载成功:', self.videoEnvMap);
                                            } catch(e) {
                                                console.error('[VideoEnv] 解析JSON失败:', e);
                                            }
                                        }
                                    }).catch(function(err) {
                                        console.error('[VideoEnv] Promise rejected:', err);
                                    });
                                } else if (result) {
                                    // 同步返回
                                    try {
                                        self.videoEnvMap = JSON.parse(result);
                                        console.log('[VideoEnv] 环境映射加载成功:', self.videoEnvMap);
                                    } catch(e) {
                                        console.error('[VideoEnv] 解析JSON失败:', e);
                                    }
                                ...
                                console.error('[VideoEnv] 调用getVideoEnvMap失败:', e);
                            }
                        } else {
                            console.warn('[VideoEnv] VideoDecoder没有getVideoEnvMap方法');
                        }
                    } else {
                        console.warn('[VideoEnv] QWebChannel中没有VideoDecoder对象');
                    }
                } catch(e) {
                    console.error('[VideoEnv] 加载视频环境映射失败:', e);
                }
            });
        }
    },
    ...
    /**
     * 播放视频（支持双模式：HTML原生播放 + Python解码播放）
    ...
    playVideo: function(videoKey) {
        ...
        console.log('[Video] videoPlayer元素存在');
        
        // 检测运行环境
        const isQtWebEngine = (typeof qt !== 'undefined' && qt.webChannelTransport);
        console.log('[Video] 环境检测:', isQtWebEngine ? 'Python/QtWebEngine' : 'HTML/浏览器');
        
        // 检查视频是否适合当前环境
        if (!this.checkVideoEnvironment(videoKey, isQtWebEngine)) {
            console.log('[Video] 环境不匹配，跳过此视频');
            // 环境不匹配，跳过此视频
            return false;
        }
        ...
        // 根据环境选择播放方式
        if (isQtWebEngine) {
            // Python环境：使用VideoDecoder获取HTTP URL播放
            this.playVideoDecoded(videoKey);
        } else {
            // HTML环境：使用原生video标签播放
    ...
    /**
     * Python环境：使用原生video标签播放WebM视频（音画完美同步）
     * @param {string} videoKey - 视频键名或文件名
     */
    playVideoDecoded: async function(videoKey) {
        if (!this.elements.videoPlayer || !this.elements.mainVideo) {
            console.error('[Video] videoPlayer或mainVideo元素不存在');
            return;
        }
        ...
        console.log('[Video] playVideoDecoded 开始, videoKey:', videoKey);
        ...
        if (!isQtWebEngine) {
            console.error('[Video] 不在QtWebEngine环境中');
            self.skipVideo();
            return;
        }
        
        console.log('[Video] QWebChannel可用，初始化视频解码器...');
        
        // 使用QWebChannel获取VideoDecoder
        new QWebChannel(qt.webChannelTransport, function(channel) {
            const videoDecoder = channel.objects.VideoDecoder;
            
            if (!videoDecoder) {
                console.error('[Video] VideoDecoder对象不存在');
                self.skipVideo();
                return;
            }
            
            console.log('[Video] VideoDecoder对象获取成功');
            
            // 使用async函数处理异步调用
            (async function() {
                try {
                    console.log('[Video] 调用 decoder.loadVideo...');
                    // 等待异步结果
                    const result = await videoDecoder.loadVideo(videoKey);
                    console.log('[Video] loadVideo返回:', result);
                    
                    const loadResult = JSON.parse(result);
                    
                    if (!loadResult.success) {
                        console.error('[Video] 视频加载失败:', loadResult.error);
                        self.skipVideo();
                        return;
                    }
                    
                    console.log('[Video] 视频加载成功，URL:', loadResult.videoUrl);
                    
                    // 使用原生video标签播放
                    self.elements.mainVideo.src = loadResult.videoUrl;
                    
                    // 添加视频播放结束事件监听器
                    self.elements.mainVideo.addEventListener('ended', function() {
                        self.elements.videoPlayer.style.display = 'none';
                        self.nextLine();
                    }, { once: true });
                    
                    // 开始播放
                    console.log('[Video] 调用play()...');
                    self.elements.mainVideo.play().catch(err => {
                        console.error('[Video] 播放失败:', err.name, err.message);
                        self.skipVideo();
                    });
                } catch (err) {
                    console.error('[Video] loadVideo异常:', err.name, err.message);
                    console.error('[Video] 错误堆栈:', err.stack);
                    self.skipVideo();
                }
            })();
        });
    ...
    saveProgress: function() {
        ...
        // 通知 Python 层：游戏进度已更新
        this.notifyPythonStorageOperation('UPDATE', 'gameProgress', 'localStorage');
    },
...
const QuickSaveManager = {
    ...
    quickSave: function() {
        ...
        try {
            ...
            // 通知 Python 层：存档已创建
            this.notifyPythonStorageOperation('CREATE', 'galgame_archives', 'localStorage');
 ...
 * @param {string} storageType - 存储类型：'localStorage' 或 'sessionStorage'
 */
gameEngine.notifyPythonStorageOperation = function(operation, key, storageType) {
    try {
        // 检查 ShioriAPI 是否可用
        if (typeof ShioriAPI !== 'undefined' && ShioriAPI.onStorageOperation) {
            const data = {
