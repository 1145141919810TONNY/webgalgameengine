...
copy /Y "bin\Release\net6.0-windows\win-x64\publish\shiori.exe" "%PUBLISH_DIR%\"
copy /Y "bin\Release\net6.0-windows\win-x64\publish\shiori.pdb" "%PUBLISH_DIR%\" 2>nul
...
echo.
echo 下一步操作:
echo 1. 将 %PUBLISH_DIR% 中的 shiori.exe 复制到 galgame-engine 目录
echo 2. 在 galgame-engine\icon 文件夹中放入你的图标文件（可选）
echo 3. 运行 shiori.exe 测试
echo.
pause
