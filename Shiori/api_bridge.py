...
class ShioriAPI(QObject):
    ...
    @pyqtSlot(str)
    def onStorageOperation(self, data_json: str):
        """
        接收来自 JavaScript 的存储操作通知
        :param data_json: JSON 格式的存储操作数据
                        {
        ...
        try:
            ...
            storage_type = data.get('storageType', 'unknown')
            ...
            print(f"\n{'='*60}")
            print(f"[{now}] [Storage Monitor] 检测到存储操作")
            ...
            # 说明：QtWebEngine 的 localStorage 使用 Chromium 内部格式
            # 数据存储在 shiori_data 目录下的 LevelDB 数据库中
            # 不是独立的 JSON 文件，但数据确实被持久化了
            storage_path = self.app_dir / "shiori_data"
            print(f"[Storage Monitor] 持久化存储路径: {storage_path}")
            print(f"[Storage Monitor] 提示: localStorage 数据以 Chromium 内部格式存储")
            print(f"[Storage Monitor]       虽然看不到独立文件，但数据已持久化到磁盘\n")
            
        except Exception as e:
            print(f"[ERROR] 解析存储操作数据失败: {e}")
            print(f"[ERROR] 原始数据: {data_json}\n")
